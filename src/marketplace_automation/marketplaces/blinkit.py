...
def process_blinkit(file_path):
    ...
    df['order_date'] = pd.to_datetime(df['order_date'], dayfirst=False, format='mixed', errors='coerce')
    df['expiry_date'] = pd.to_datetime(df['expiry_date'], dayfirst=False, format='mixed', errors='coerce')
